from flask import Flask, request, render_template, send_file import os import uuid from arg_analyzer.core import generate_argx_and_heatmap

UPLOAD_FOLDER = "/tmp/uploads" MAX_PDFS = 30
...
@app.route("/", methods=["GET", "POST"]) def index(): def format_pdf_display_name(filename): match = re.search(r'(\d{4}-\d{2}-\d{2})', filename) date_str = match.group(1) if match else "Unknown" return f"ARG_{date_str}.pdf", filename
...
@app.route("/download/<filename>") def download(filename): print(f"[DOWNLOAD] {filename} requested") file_path = os.path.join("/tmp", filename)
